...
当您写入 `key` 和 `key_max` 两个变量时，插件会自动为您计算 `key_pct` 占位符（即百分比，范围 `0-100`），非常适合渲染血条、蓝条和经验值：

| 变量对 | 自动生成百分比占位符 | 用途示例 |
|---|---|---|
| `hp` 与 `hp_max` | `{{hp_pct}}` | 活力/生命值进度条 |
| `mp` 与 `mp_max` | `{{mp_pct}}` | 魔力值进度条 |
| `sp` 与 `sp_max` | `{{sp_pct}}` | 体力值进度条 |
| `exp` 与 `exp_max` | `{{exp_pct}}` | 经验值/好感度进度条 |

### 3. 系统内置全局变量 (System Variables)
